...
class YAMLBuilder:
    ...
    DEFAULT_HOOK_VERSIONS: ClassVar[Dict[str, str]] = {
        ...
        "https://github.com/RobertCraigie/pyright-python": "v1.1.391",
        "https://github.com/abravalheri/validate-pyproject": "v0.13.0",
        "https://github.com/gitleaks/gitleaks": "v8.22.1",
        "https://github.com/psf/black": "23.12.1",
        "https://github.com/pycqa/isort": "5.13.2",
        "https://github.com/pycqa/flake8": "6.1.0",
        "https://github.com/pre-commit/mirrors-prettier": "v3.1.0",
        "https://github.com/pre-commit/mirrors-eslint": "v8.56.0",
        "https://github.com/thibaudcolas/curlylint": "v0.13.1",
        "https://github.com/pre-commit/mirrors-csslint": "v1.0.5",
        "https://github.com/adrienverge/yamllint": "v1.33.0",
        "https://github.com/markdownlint/markdownlint": "v0.12.0",
    ...
    def _load_hook_versions(self) -> Dict[str, str]:
        ...
        1. ~/.pre-commit-starter/versions.json
        ...
        user_config_path = Path.home() / ".pre-commit-starter" / "versions.json"
    ...
    def build_config(self, detected_techs: Union[List[str], Set[str]]) -> str:
        ...
        header = [
            "# Pre-commit configuration generated by pre-commit-starter",
            f"# Technologies detected: {', '.join(sorted(techs))}",
            "# To install: pre-commit install",
            "# To update: pre-commit autoupdate",
        ]

        if self.custom_repos:
            header.append("# Includes custom hooks from .pre-commit-starter-hooks.yaml")
        ...
        yaml_str = yaml.dump(config, sort_keys=False, Dumper=PreCommitDumper)

        # Return configuration with header
        return header_str + yaml_str

    def _load_user_hook_versions(self) -> Dict[str, str]:
        """Load hook versions from user's config in $HOME/.pre-commit-starter/hook_versions.json."""
        user_config_path = Path.home() / ".pre-commit-starter" / "hook_versions.json"
        if user_config_path.exists():
            try:
                with open(user_config_path, encoding="utf-8") as f:
                    return json.load(f)
